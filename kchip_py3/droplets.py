...
def find_droplets(config,image,threshold=0.53,show=1):
    ...
    hough_radii = np.arange(50./config['image']['pixel_size'],80/config['image']['pixel_size'])
    hough_res = hough_circle(edges,hough_radii)
    # 2b) Take the maximum to avoid redundancies at different radii
    hough_res = np.max(hough_res,axis=0)
    radii_val = int(50/config['image']['pixel_size'])

    # 2c) Find the peaks in the accumulators
    accum, cx, cy, rad = hough_circle_peaks([hough_res],[radii_val],threshold=threshold,min_xdistance=15,min_ydistance=15)
