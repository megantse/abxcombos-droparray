...
def clip_image(image, size=(101,101)):
    ...
    for axis in range(len(image.shape)):
        ...
        slices.append(slice(int(diff/2),int(-diff/2)))

    return image[tuple(slices)] # updated but not checked
    # before:
    # return image[slices]
...
def slice_mask(config,rotation):
    ...
    slicer = lambda x,y,error: [slice(int(max(shifty(x,y)+topleft[1]-error,0)), \
        int(max(shifty(x,y)+FOV_size+topleft[1]+error,0))),\
        slice(int(max(shiftx(x,y)+topleft[0]-error,0)), int(max(0,shiftx(x,y)+FOV_size+topleft[0]+error)))]
    return slicer
...
def pad_axis(img,axis,pad_width):
    if pad_width != 0:
        if axis == 0:
            ...
            new_img[int(pad_width/2):int(-pad_width/2),:]=img
        else:
